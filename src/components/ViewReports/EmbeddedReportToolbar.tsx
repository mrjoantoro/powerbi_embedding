  ...
  render() {
    return (
      <div id="embedded-report-toolbar" >
        <div>
          <button onClick={() => this.toggleReportEditMode()}>Editar Reporte</button>
          <button onClick={() => this.enterFullScreenMode()} >Pantalla Completa</button>
          <button onClick={() => this.printReport()} >Imprimir</button>
        </div>
      </div>);
  }
